...
```shell
 docker run --privileged debian sh -c "apt-get update && apt install libcap2-bin -y && capsh --print|grep 'Bounding set'|tr ',' '\n'|wc -l"
```

# 5 Escape from container

```shell
 df -h
 docker run -it --privileged alpine ash
 apk add -q libcap
